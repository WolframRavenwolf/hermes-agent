...
class TavilyWebSearchProvider(WebSearchProvider):
    ...
    def extract(self, urls: List[str], **kwargs: Any) -> List[Dict[str, Any]]:
        """Extract allowed URLs and return exactly one result per input slot."""
        ...
        try:
            ...
            for url in urls:
                ...
                if blocked:
                    documents[url] = {
                        ...
                        "blocked_by_policy": {
                            key: blocked[key] for key in ("host", "rule", "source")
                        },
                    }
                else:
            ...
            if allowed_urls:
                ...
                force_keyless = use_keyless("tavily", api_key)
                if not force_keyless and not api_key:
                    raise ValueError(_missing_key_error("extract"))

                logger.info(
                    "Tavily %sextract: %d URL(s)",
                    "keyless " if force_keyless else "", len(allowed_urls),
                )
                raw = _tavily_request(
                    "extract", {"urls": allowed_urls, "include_images": False},
                    api_key="" if force_keyless else api_key,
                )
                # Tavily groups successes before failures. Match by URL before
                # the wrapper's positional cache write; never guess a batch URL.
                for doc in _normalize_tavily_documents(
                    raw, fallback_url=allowed_urls[0] if len(allowed_urls) == 1 else "",
                ):
                    if doc["url"] in allowed_urls:
        ...
        # Missing responses and duplicate inputs retain their original slots;
        # blocked entries survive missing credentials and transport failures.
        return [
            dict(documents.get(u, {"url": u, "title": "", "content": "", "error": error}))
            for u in urls
        ]
