...
class MattermostAdapter(BasePlatformAdapter):
    ...
    async def _api_post(
        ...
        try:
            async with self._session.post(
                ...
                status = self._last_post_status = resp.status
                if resp.status >= 400:
                    try:
                        body = await resp.text()
                    ...
                    return {"_post_rejected": True, "_post_status": status, "_post_error": body}
                data = await resp.json()
                return data if data.get("id") else {"_delivery_uncertain": True, "content_uncertain": True}
        except (aiohttp.ClientError, asyncio.TimeoutError, ValueError) as exc:
            self._last_post_error = str(exc)
            logger.error("MM API POST %s network error: %s", path, exc)
            return ({"_post_rejected": True, "_post_status": status}
                    if status is not None and status >= 400 else
                    {"_delivery_uncertain": True, "content_uncertain": True})
    ...
    async def _post_preserving_thread(
        ...
        if (data.get("id") or data.get("_delivery_uncertain")
                or "root_id" not in payload):
            return data
        ...
        if data.get("_post_rejected"):
            ...
            broken_root = data.get("_post_status") in {400, 404} and any(
                marker in body for marker in ("root_id", "rootid", "root id", "thread", "post")
            ) and any(marker in body for marker in ("invalid", "not found", "does not exist", "missing"))
        else:
            broken_root = self._last_post_failure_is_broken_thread_root()
        if not broken_root:
            return data
    ...
    async def _post_chunked_payload(
        ...
        *,
        ...
        source_chunks = self._source_chunks(source) if source is not None else None
        ...
        for chunk_index, chunk_payload in enumerate(payloads):
            ...
            try:
                ...
                # A timeout/JSON error can follow an acknowledged earlier post.
                # Keep its receipt and never replay a non-idempotent send.
                data = {"_delivery_uncertain": True, "content_uncertain": True}
            if not data or "id" not in data:
                return SendResult(
                    ...
                    raw_response={
                        **(data or {}), "message_ids": tuple(message_ids),
                        **({"source_confirmed_prefix": source[:confirmed],
                            "source_attempted_prefix": source[:confirmed + (
                                len(source_chunks[chunk_index]) if (data or {}).get("content_uncertain") else 0
                            )]} if source_chunks is not None else {}),
                    },
                )
            if source_chunks is not None:
    ...
    def _source_chunks(self, content: str) -> List[str]:
        ...
        if len(self.format_message(content)) <= self.max_post_length:
            return [content]
        return [content[i:i + self.max_post_length]
                for i in range(0, len(content), self.max_post_length)]
    ...
    async def edit_message(
        ...
        *,
        ...
        payloads = [_with_mentions_disabled({"message": self.format_message(chunk)}) for chunk in chunks]
        ...
        for index, chunk_payload in enumerate(payloads[1:], 1):
            post_payload = dict(chunk_payload)
            post_payload["channel_id"] = chat_id
            if resolved_root:
                post_payload["root_id"] = resolved_root
            try:
                posted = await self._post_preserving_thread(chat_id, post_payload, metadata)
            except asyncio.CancelledError:
                raise
            except Exception:
                posted = {"_delivery_uncertain": True, "content_uncertain": True}
            if not posted or "id" not in posted:
                last_id = continuation_ids[-1] if continuation_ids else str(data["id"])
                delivered_prefix = content[:confirmed]
                attempted = confirmed + (len(chunks[index]) if (posted or {}).get("content_uncertain") else 0)
                return SendResult(
                    success=False,
                    message_id=last_id,
                    ...
                    raw_response={
                        **(posted or {}),
                        "source_confirmed_prefix": delivered_prefix,
            ...
            confirmed += len(chunks[index])

        return SendResult(
            success=True,
            raw_response={"source_confirmed_prefix": content, "source_attempted_prefix": content},
