        ...
        try:
            ...
            try:
                ...
                retryable = isinstance(
                    exc, (WhatsAppBridgeBusyError, WhatsAppBridgeUnavailableError)
                )
                self._set_fatal_error(
                    "whatsapp_dependencies_unavailable" if retryable else "whatsapp_npm_install_failed",
                    str(exc) if retryable else f"WhatsApp bridge npm install failed: {exc}",
                    retryable=retryable,
                )
                return False
