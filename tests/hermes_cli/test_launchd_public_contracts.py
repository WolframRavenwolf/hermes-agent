...
@pytest.mark.parametrize("unsafe_pending", [False, True])
def test_deferred_update_does_not_accept_stale_fallback(service, monkeypatch, unsafe_pending):
    from hermes_cli.update_cmd_fleet import _restart_launchd_gateway_after_update

    fallback = service.plist.parent / "unsupported-marker"
    fallback.write_text("previous detached fallback")
    monkeypatch.setattr(gateway, "_launchd_unsupported_marker_path", lambda: fallback)
    monkeypatch.setattr("gateway.status.get_running_pid", lambda: 4242)
    clock = [0.0]
    monkeypatch.setattr(gateway, "time", SimpleNamespace(
        time=lambda: 0, monotonic=lambda: clock[0],
    ...
    monkeypatch.setattr(gateway.subprocess, "Popen", submit)
    service.pid = 0  # Helper creation does not establish launchd supervision.
    restarted, failed = _restart_launchd_gateway_after_update()
    assert restarted == []
    assert failed == [gateway.get_launchd_label()]
    assert len(submitted) == 1
    assert service.marker.exists() and fallback.exists()
    assert not any(cmd[1] in ("bootout", "bootstrap", "kickstart") for cmd in service.calls)
...
@pytest.mark.parametrize("entry,failure", [
    ("refresh_launchd_plist_if_needed", failure)
    for failure in ("unobserved", "bootstrap_timeout", "bootout_timeout", "retirement")
] + [
    ("launchd_install", failure)
    for failure in ("unobserved", "bootstrap_timeout", "retirement")
])
def test_uncertain_activation_keeps_published_definition(service, monkeypatch, missing, entry, failure):
    if missing:
        service.plist.unlink()
    ...
    def run(cmd, **kwargs):
        if (failure == "bootstrap_timeout" and cmd[1] == "bootstrap"
                or failure == "bootout_timeout" and cmd[1] == "bootout"):
            raise subprocess.TimeoutExpired(cmd, 30)
        if failure == "unobserved" and cmd[1] == "list":
            return SimpleNamespace(returncode=1, stdout="", stderr="not visible in caller domain")
        return original_run(cmd, **kwargs)

    def unlink(path, *args, **kwargs):
        if failure == "retirement" and path == service.marker:
            raise OSError("marker retirement unavailable")
    ...
    with pytest.raises(launchd_reload.LaunchdReloadError):
        getattr(gateway, entry)()
    assert service.plist.read_bytes() == b"new definition"
    assert stat.S_IMODE(service.plist.stat().st_mode) == (0o600 if missing else 0o640)
    assert service.marker.exists()
...
@pytest.mark.parametrize("code", [5, 125])
@pytest.mark.parametrize("spawn_succeeds", [False, True])
def test_updater_accounts_for_current_detached_fallback(service, monkeypatch, code, spawn_succeeds):
    from hermes_cli.update_cmd_fleet import _restart_launchd_gateway_after_update
    ...
    monkeypatch.setattr(gateway, "_launchd_unsupported_marker_path", lambda: fallback)
    detached = []
    monkeypatch.setattr(gateway, "_spawn_detached_gateway",
                        lambda: detached.append(True) or spawn_succeeds)
    verify = gateway.wait_for_launchd_gateway_supervision
    monkeypatch.setattr(gateway, "wait_for_launchd_gateway_supervision",
                        lambda **kwargs: verify(timeout=0, **kwargs))
    if spawn_succeeds:
...
@pytest.mark.parametrize("pid", [0, 5150])
def test_public_retry_consumes_failed_install_state(failed_initial_install, monkeypatch, entry, pid):
    from hermes_cli.update_cmd_fleet import _restart_launchd_gateway_after_update

    service = failed_initial_install
    service.pid = pid
    native_all_calls = []
    if entry == "restart_all":
        # A retained plist follows the native all-profile stop path before retry.
        # Only process I/O is mocked; manual run/stop-profile fallback stays forbidden.
        monkeypatch.setattr(gateway, "kill_gateway_processes",
                            lambda **kw: native_all_calls.append(("kill", kw)) or 0)
        monkeypatch.setattr(gateway, "_wait_for_gateway_exit",
                            lambda **kw: native_all_calls.append(("wait", kw)) or True)
    if entry == "updater":
        if pid == 0:
            assert _restart_launchd_gateway_after_update() == ([], [gateway.get_launchd_label()])
        else:
...
def test_rollback_directory_sync_failure_still_propagates(service, monkeypatch, missing):
    ...
    def fail_submit(*args, **kwargs):
        ...
        raise OSError("submission failed before actuation")

    monkeypatch.setattr(gateway.subprocess, "Popen", fail_submit)

    def fsync_checked(fd):
        if submission_failed and stat.S_ISDIR(os.fstat(fd).st_mode):
...
def test_native_helper_only_clears_matching_positive_pid_attempt(service, monkeypatch, pid, newer, cleanup_failure):
    fallback = service.plist.parent / "unsupported-marker"
    if cleanup_failure:
        fallback.mkdir()  # Removal fails; pending authority must remain.
    else:
        fallback.write_text("previous detached fallback")
    monkeypatch.setattr(gateway, "_launchd_unsupported_marker_path", lambda: fallback)
    monkeypatch.setattr("gateway.status.get_running_pid", lambda: 4242)
    ...
    incomplete = pid == 0 or newer or cleanup_failure
    assert service.marker.exists() is incomplete, result.stderr
    assert fallback.exists() is incomplete, result.stderr
    if not incomplete:
        service.pid = 0
        assert gateway.wait_for_launchd_gateway_supervision(timeout=0) is False
    if newer:
        assert service.marker.read_text() == "newer attempt"
