...
def _install_unit_test_provider_profile(monkeypatch) -> ProviderProfile:
    ...
    monkeypatch.setattr(
        provider_registry, "get_provider_profile", fake_get_provider_profile
    )
    config = auth_mod.ProviderConfig(
        id=profile.name,
    ...
    monkeypatch.setitem(auth_mod.PROVIDER_REGISTRY, profile.name, config)
    for alias in profile.aliases:
        monkeypatch.setitem(auth_mod.PROVIDER_REGISTRY, alias, config)
    return profile
...
def test_normalize_provider_does_not_trigger_profile_discovery(monkeypatch):
    ...
    monkeypatch.setattr(
        provider_registry, "get_provider_profile", fail_get_provider_profile
    )

    assert (
        providers_mod.normalize_provider("missing-profile-provider")
        == "missing-profile-provider"
    )
