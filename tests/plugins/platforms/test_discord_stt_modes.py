...
def test_buffer_retirement_preserves_active_mapped_decoder(decoded_packet_receiver, retirement):
    receiver, receive = decoded_packet_receiver
...
def test_single_human_presence_does_not_infer_unmapped_packet_owner(decoded_packet_receiver):
    receiver, receive = decoded_packet_receiver
    receiver._vc.channel.members = [SimpleNamespace(id=999), SimpleNamespace(id=42)]
    receive(100, b"\x01\x00\x02\x00" * 24000)
    assert receiver.flush_pending() == []
