...
_ADAPTER_OPTIONAL_DEPENDENCIES = {
    "plugins.platforms.telegram.adapter": ("telegram",),
    "plugins.platforms.slack.adapter": ("aiohttp", "slack_bolt", "slack_sdk"),
    "plugins.platforms.mattermost.adapter": ("aiohttp",),
...
def test_inert_description_stays_inert_after_target_platform_formatting(
    ...
    for dependency in _ADAPTER_OPTIONAL_DEPENDENCIES[module_name]:
        try:
            importlib.import_module(dependency)
        except ModuleNotFoundError as exc:
            if exc.name != dependency:
                raise
            pytest.skip(f"optional platform dependency is unavailable: {dependency}")
    adapter_module = importlib.import_module(module_name)
    adapter_type = getattr(adapter_module, adapter_name)
    formatted = adapter_type.__new__(adapter_type).format_message(
        f"Running code: {preview.text}"
    )
...
def test_browser_exec_preview_is_unchanged():
    ...
    assert (
        ctx.progress_queue.get_nowait() == '🌐 browser_exec: "# Browser label return 1"'
    )
