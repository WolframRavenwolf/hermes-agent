...
@pytest.mark.parametrize("network_error", [True, False], ids=["network", "generic"])
@pytest.mark.asyncio
async def test_standalone_error_redaction_preserves_receipts(network_error, monkeypatch):
    """Shared error sanitization must compose with acknowledged chunk receipts."""
    import aiohttp
    from gateway.config import PlatformConfig
    from plugins.platforms.mattermost.adapter import _standalone_send

    config = PlatformConfig(token="fixture-token", extra={
        "url": "https://mattermost.example", "max_post_length": 500})
    session = MagicMock()
    session.__aenter__ = AsyncMock(return_value=session)
    error_type = aiohttp.ClientError if network_error else RuntimeError
    session.__aexit__ = AsyncMock(side_effect=error_type(
        "close failed: https://mattermost.example/api?signature=fixture-private-signature"))
    posts = []
    ...
    result = await _standalone_send(config, "channel", "x" * 600)

    assert not result["success"]
    assert result["platform"] == "mattermost" and result["chat_id"] == "channel"
    assert result["message_ids"] == ["post-1", "post-2"]
    assert result["message_id"] == "post-2"
    assert len(posts) == 2
    assert "fixture-private-signature" not in result["error"]
