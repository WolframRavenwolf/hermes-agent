...
class TestMattermostAuthoritativeFallback:
    ...
    @pytest.mark.parametrize("upload_outcome, fail_at, caption_outcome", [
        (413, 1, "ok"), (503, 2, "ok"), ("missing_id", 1, "ok"),
        ("timeout", 1, "ok"), ("timeout", 7, "ok"),
        (413, 1, "timeout"), (503, 1, "empty"), ("cancelled", 1, "ok"),
    ])
    @pytest.mark.asyncio
    async def test_upload_failure_preserves_only_pending_caption(
        self, tmp_path, upload_outcome, fail_at, caption_outcome,
    ):
        import asyncio
        adapter, posts, events = self.transport(["ok", caption_outcome])
        del adapter._upload_file  # Exercise the real upload helper and local-file reads.
        original_post = adapter._session.post.side_effect
        uploads = []
        ...
        def post(url, **kwargs):
            ...
            if len(uploads) == fail_at:
                if isinstance(upload_outcome, int):
                    response.status = upload_outcome
                    response.text.return_value = "upload rejected"
                elif upload_outcome == "missing_id":
                    response.json.return_value = {"file_infos": [{}]}
                else:
                    response.__aenter__.side_effect = (
                        asyncio.CancelledError() if upload_outcome == "cancelled" else TimeoutError("upload lost"))
            return response

        adapter._session.post.side_effect = post
        caption = "  explicit @here\n![keep](https://example.com/image)\t" + "x" * 550 + "  "
        delivery = adapter.send_multiple_images(
            "channel", self.images(tmp_path, 8), caption=caption,
            metadata={"thread_id": "root", "mattermost_explicit_thread": True})
        if upload_outcome == "cancelled":
            with pytest.raises(asyncio.CancelledError):
                await delivery
            assert len(uploads) == 1 and not posts
            return
        result = await delivery
        assert "".join(p["message"] for p in posts) == caption
        assert len(posts) == 2
        assert len(uploads) == fail_at  # No retry or later media work, including partial current uploads.
        assert all(len(p["message"]) <= 500 and p["root_id"] == "root"
                   and p["props"]["disable_mentions"] for p in posts)
        delivered = 5 if fail_at > 5 else 0
        assert [p.get("file_ids") for p in posts] == [None, [f"file-{i}" for i in range(1, 6)] if delivered else None]
        ids = ["post-1", "post-2"] if caption_outcome == "ok" else ["post-1"]
        ...
        assert result.raw_response["total_media"] == 8
        assert result.raw_response["delivered_media"] == delivered
        assert result.raw_response["failed_media"] == 8 - delivered
    ...
    async def test_upload_rejection_preserves_later_tuple_caption(self, tmp_path, caption_index):
        adapter, posts, events = self.transport(["ok"])
        del adapter._upload_file
        original_post = adapter._session.post.side_effect
        uploads = []
        ...
        adapter._session.post.side_effect = post
        adapter._load_batch_image = AsyncMock(wraps=adapter._load_batch_image)
        images = self.images(tmp_path, 1)
        images.extend((str(tmp_path / f"missing-{i}.png"), "") for i in range(1, caption_index + 1))
        images[-1] = (images[-1][0], "B context")
        result = await adapter.send_multiple_images(
            "channel", images, metadata={"thread_id": "root", "mattermost_explicit_thread": True})

        assert [p["message"] for p in posts] == ["B context"]
        assert posts[0]["root_id"] == "root"
        assert posts[0]["props"]["disable_mentions"] is True
        assert not posts[0].get("file_ids")
        assert len(uploads) == 1
        adapter._load_batch_image.assert_awaited_once()
        assert result.raw_response["message_ids"] == ["post-1"]
        assert result.message_id == "post-1"
        assert result.raw_response["delivered_media"] == 0
        assert result.raw_response["failed_media"] == len(images)
        assert result.success is False
        assert result.error and "upload" in result.error.lower()
