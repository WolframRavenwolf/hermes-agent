...
def _isolated_npm_discovery(monkeypatch):
    # Installer calls are mocked in these unit tests, not dependent on host PATH.
    # Explicit missing-executable tests override this fixture themselves.
    monkeypatch.setattr(whatsapp_common, "find_node_executable", lambda name: "/usr/bin/npm")
...
def test_activation_control_exception_restores_exact_prior_state(
    ...
    before = _file_snapshot(modules)
    monkeypatch.setattr(whatsapp_common.subprocess, "run", _successful_npm_ci([]))
    real_replace = os.replace
    real_fingerprint = whatsapp_common.whatsapp_bridge_dependency_fingerprint
    real_fresh = whatsapp_common._whatsapp_bridge_dependencies_are_fresh
    ...
    def fresh(path, expected_fingerprint):
        result = real_fresh(path, expected_fingerprint)
        if promoted:
            assert result is True
            interrupt("verify-fresh")
        return result

    monkeypatch.setattr(whatsapp_common.os, "replace", replace)
    monkeypatch.setattr(whatsapp_common, "whatsapp_bridge_dependency_fingerprint", fingerprint)
    monkeypatch.setattr(whatsapp_common, "_whatsapp_bridge_dependencies_are_fresh", fresh)
    ...
    assert _file_snapshot(modules) == before
    assert not list(bridge.glob(".node_modules.staging-*"))
    assert not list(bridge.glob(".node_modules.backup-*"))
