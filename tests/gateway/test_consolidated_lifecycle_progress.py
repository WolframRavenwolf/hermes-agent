...
@pytest.mark.asyncio
async def test_native_task_cards_keep_compaction_on_standalone_status_rail(monkeypatch):
    ...
    class StatusAdapter(CaptureAdapter):
        def __init__(self):
            super().__init__()
    ...
    ctx._status_chat_id = ctx.source.chat_id
    ctx._loop_for_step = asyncio.get_running_loop()
    turn._status_callback_sync("lifecycle", COMPACTION_STATUS)

    assert ctx.progress_queue.empty(), "native task-card consumer ignores text entries"
    await asyncio.wait_for(adapter.delivered.wait(), timeout=1)
    assert adapter.sent == [COMPACTION_STATUS]
...
@pytest.mark.asyncio
@pytest.mark.parametrize("outcome", ["declined_code", "declined_text", "failed_edit", "replacement", "partial_edit", "same_id", "rollover"])
async def test_residual_terminal_receipt_controls_native_completion_and_cleanup(monkeypatch, tmp_path, outcome):
    import tests.gateway.test_run_progress_topics as progress

    captured = {}
    class Adapter(progress.ProgressCaptureAdapter):
        SUPPORTS_MESSAGE_EDITING = True
        MAX_MESSAGE_LENGTH = 100 if outcome == "rollover" else 4000
        ...
        async def send(self, chat_id, content, reply_to=None, metadata=None):
            ...
            self.operations.append(("send", mid, content))
        ...
        async def edit_message(self, chat_id, message_id, content, **kwargs):
            ...
            if "Session auto-reset" not in content:
            ...
            if outcome == "declined_text":
                return SendResult(success=False, message_id=message_id,
                                  error="telegram egress declined: target is not an approved destination for this connection")
            if outcome == "failed_edit":
                return SendResult(success=False, message_id=message_id, error="editing unavailable")
            mid = message_id if outcome == "same_id" else "m2"
            self.visible[mid] = content
            if outcome == "partial_edit":
                self.visible[mid] = content[:80]
                return SendResult(success=False, message_id=message_id,
                                  raw_response={"message_ids": [mid]}, error="continuation failed")
            return SendResult(success=True, message_id=mid)
    ...
    async def send_and_signal(self, *args, **kwargs):
        result = await original_send(self, *args, **kwargs)
        first_send.set()
        return result
    monkeypatch.setattr(Adapter, "send", send_and_signal)

    class Agent(progress.FakeAgent):
        def run_conversation(self, *args, **kwargs):
            self.tool_progress_callback("tool.started", "terminal", "temporary progress", {})
            assert first_send.wait(3), "native progress did not send before model completion"
            return {"final_response": "context exhausted", "compression_exhausted": True,
                    "messages": [], "api_calls": 1}

    adapter, result = await progress._run_with_agent(
        monkeypatch, tmp_path, Agent, session_id="terminal-receipts",
        adapter_cls=Adapter, defer_terminal_lifecycle_progress=True,
        config_data={"display": {"tool_progress": "all", "long_running_notifications": False,
                                 "cleanup_progress": True}},
    )
    captured["terminal_start"] = len(adapter.operations)
    complete = result["_complete_lifecycle_progress"]
    handler, key = _make_handler_runner(tmp_path, adapter)
    monkeypatch.setenv("MATTERMOST_HOME_CHANNEL", "channel-1")
    event, source = _handler_event_and_source()
    handler._run_agent = AsyncMock(return_value=result)
    response = await handler._handle_message_with_agent(event, source, key, 1)
    delivery = await complete()
    refused = outcome.startswith("declined")
    if refused:
        assert [op for op, _, _ in adapter.operations[captured["terminal_start"]:]] == ["edit"], adapter.operations
        assert isinstance(delivery, SendResult) and not delivery.success
        assert response == ""
    else:
        assert delivery is (outcome != "partial_edit")
        assert bool(response) is (outcome == "partial_edit")
    # Exercise the production post-delivery callback registered before the
    # deferred terminal response changed the receipt-bearing message IDs.
    cleanup_key = "agent:main:telegram:group:-1001:17585"
    cb = adapter.pop_post_delivery_callback(cleanup_key)
    assert cb is not None
    finished = cb()
    if hasattr(finished, "__await__"):
        await finished
    await asyncio.sleep(0.05)
    assert adapter.deleted == ([] if outcome == "same_id" else ["m1"])
    assert set(adapter.visible) == (set() if refused else {"m1"} if outcome == "same_id" else {"m2"})


@pytest.mark.asyncio
@pytest.mark.parametrize("refuse_on", ["send", "edit"])
async def test_residual_destination_refusal_closes_later_progress_and_cancel_drain(refuse_on):
    class Adapter(CaptureAdapter):
        def __init__(self):
            super().__init__()
            self.calls = []
        async def send(self, chat_id, content, **kwargs):
            ...
            if refuse_on == "send":
                return SendResult(success=False, error="relay egress declined: target is not an approved destination for this connection")
    ...
    try:
        ...
        ctx.progress_queue.put(("__terminal__", "second", second))
        delivery = await asyncio.wait_for(second, 3)
        ctx.progress_queue.put("later progress")
    finally:
        task.cancel()
        await asyncio.gather(task, return_exceptions=True)
    assert isinstance(delivery, SendResult) and not delivery.success
    assert [op for op, _ in adapter.calls] == (["send"] if refuse_on == "send" else ["send", "edit"])



@pytest.mark.asyncio
@pytest.mark.parametrize("method", ["send", "send_for_platform", "edit_message"])
async def test_native_relay_keeps_structured_receipt_at_conversion(method):
    from gateway.config import PlatformConfig
    from gateway.relay.adapter import RelayAdapter
    from tests.gateway.relay.stub_connector import StubConnector
    from tests.gateway.relay.test_relay_live_cards import make_desc

    payload = {"success": False, "code": "egress_declined", "message_ids": ["partial1"]}
    ...
    desc = make_desc(platform="telegram", supported_ops=("send", "edit"))
    adapter = RelayAdapter(PlatformConfig(), desc, transport=Connector(desc))
    if method == "send_for_platform":
        result = await adapter.send_for_platform("telegram", "chat1", "body")
    elif method == "edit_message":
        result = await adapter.edit_message("chat1", "m1", "body")
    else:
        result = await adapter.send("chat1", "body")
    assert not result.success
    assert result.raw_response == payload


@pytest.mark.asyncio
async def test_native_relay_structured_and_text_refusals_stop_progress():
    """Real relay receipts retain refusal identity through the progress loop."""
    from gateway.config import PlatformConfig
    from gateway.relay.adapter import RelayAdapter
    from tests.gateway.relay.stub_connector import StubConnector
    from tests.gateway.relay.test_relay_live_cards import make_desc

    desc = make_desc(platform="telegram", supported_ops=("send", "edit", "delete", "typing"))
    for outcome in ("code", "text"):
        class Connector(StubConnector):
            async def send_outbound(self, action, *, platform=None):
                self.sent.append(action)
                if action["op"] == "edit":
        ...
        try:
            ...
            ack = asyncio.get_running_loop().create_future()
            ctx.progress_queue.put(("__terminal__", "second", ack))
            delivery = await asyncio.wait_for(ack, 3)
        finally:
            task.cancel()
            await asyncio.gather(task, return_exceptions=True)
        assert isinstance(delivery, SendResult) and not delivery.success
        assert [action["op"] for action in wire.sent if action["op"] in {"send", "edit"}] == ["send", "edit"]
