...
def _no_external_model_catalog(monkeypatch):
    # Real constructors/request builders below; mock only catalog HTTP I/O.
...
def _sqlite_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> SessionStore:
    ...
    monkeypatch.setattr(
        ...
        lambda: real_session_db(db_path=tmp_path / "state.db"),
    )
    store = SessionStore(tmp_path / "sessions", GatewayConfig())
    assert isinstance(store._db, real_session_db)
    return store
...
def test_manual_fallback_parameter_preserves_existing_positional_signatures():
    ...
    assert list(inspect.signature(AIAgent.__init__).parameters)[-10:] == expected_tail
    assert list(inspect.signature(init_agent).parameters)[-10:] == expected_tail
...
async def test_fallback_route_labels_are_force_redacted(tmp_path, monkeypatch):
    ...
    chain[0]["provider"] = "token=provider-fixture-secret\nroute-label"
    store = _store(tmp_path, monkeypatch)
    runner = _runner(store, chain)
    source = _source()
    monkeypatch.setattr(
    ...
    assert "provider-fixture-secret" not in enabled + status
    assert "\nroute-label" not in enabled + status
...
def test_native_metadata_clock_and_committed_primary_survive_mirror_failure(tmp_path, monkeypatch):
    store = _sqlite_store(tmp_path, monkeypatch)
    entry = store.get_or_create_session(_source())
    timestamp = entry.updated_at
    store.set_model_override(entry.session_key, {"model": "primary", "provider": "openrouter", "api_key": "never-persist"})
    monkeypatch.setattr(store, "_save_sessions_json", MagicMock(side_effect=OSError("mirror failed")))
    assert store.set_session_metadata(entry.session_key, _MANUAL_KEY, 0)
    assert entry.updated_at == timestamp
    reopened = SessionStore(store.sessions_dir, store.config)
    assert reopened._db is not None
    assert reopened.get_session_metadata(entry.session_key, _MANUAL_KEY) == 0
    assert reopened.get_model_override(entry.session_key) == {"model": "primary", "provider": "openrouter"}
    assert store.delete_session_metadata(entry.session_key, _MANUAL_KEY)
    assert entry.updated_at == timestamp
    assert SessionStore(store.sessions_dir, store.config).get_session_metadata(entry.session_key, _MANUAL_KEY) is None


def test_failed_reset_does_not_unlock_persisted_manual_route(tmp_path, monkeypatch):
    store = _sqlite_store(tmp_path, monkeypatch)
    entry = store.get_or_create_session(_source())
    assert store.set_session_metadata(entry.session_key, _MANUAL_KEY, 0)
    assert store.set_session_metadata(entry.session_key, "compression_exhausted", True)
    monkeypatch.setattr(store._db, "replace_gateway_routing_entries", MagicMock(side_effect=OSError("primary failed")))
    ...
    assert reopened._db is not None
    assert reopened.get_session_metadata(entry.session_key, _MANUAL_KEY) == 0
    assert reopened.get_session_metadata(entry.session_key, "compression_exhausted") is True
    assert reopened.get_or_create_session(_source()).session_id == entry.session_id


@pytest.mark.asyncio
@pytest.mark.parametrize("authorized,mutation", [(False, False), (True, True), (True, False)])
async def test_native_inbound_fallback_dispatch_honors_auth_and_route_claim(authorized, mutation, tmp_path, monkeypatch):
    from tests.gateway.test_compression_exhaustion_reset_policy import _make_runner
    runner, _ = _make_runner("none")
    store = _sqlite_store(tmp_path, monkeypatch)
    runner.session_store = store
    runner._is_user_authorized = lambda source: authorized
    runner._refresh_fallback_model = lambda: _chain()
    runner._scale_to_zero_note_real_inbound = MagicMock()
    source = _source()
    entry = store.get_or_create_session(source)
    if mutation:
        assert runner._try_claim_routing_mutation(entry.session_key)
    monkeypatch.setattr(gateway_run, "_resolve_fallback_entry_agent_kwargs", lambda item: (item["model"], {"provider": item["provider"]}))
    response = await runner._handle_message(_event("/fallback on", source=source))
    if authorized and not mutation:
        assert "Manual fallback: ON" in response
        assert store.get_session_metadata(entry.session_key, _MANUAL_KEY) == 0
    else:
        assert store.get_session_metadata(entry.session_key, _MANUAL_KEY) is None
    assert entry.session_key not in runner._running_agents
    assert not runner._queue_depth(entry.session_key)


def test_manual_policy_does_not_replace_native_setup_fallback_policy(monkeypatch):
    from run_agent import AIAgent
    from unittest.mock import patch

    fallback_client = SimpleNamespace(
        api_key="fallback-fixture", base_url="https://openrouter.ai/api/v1",
        _custom_headers={}, default_headers={}, _default_headers={},
    )
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    with patch("run_agent.get_tool_definitions", return_value=[]), patch("run_agent.check_toolset_requirements", return_value={}), patch("run_agent.OpenAI"), patch(
        "agent.auxiliary_client.resolve_provider_client",
        side_effect=[(None, None), (fallback_client, "gpt-4o")],
    ):
        agent = AIAgent(
            provider="openai", model="gpt-5.4", api_mode="chat_completions",
...
def test_alias_advance_uses_existing_real_agent_regression():
    from tests.run_agent.test_provider_fallback import TestFallbackChainAdvancement
    TestFallbackChainAdvancement().test_manual_entry_alias_is_skipped_before_advancing_to_next_fallback()


def test_native_metadata_reaches_real_constructor_and_p19_wire_boundary(tmp_path, monkeypatch):
    from run_agent import AIAgent
    from agent.chat_completion_helpers import build_api_kwargs
    from unittest.mock import patch

    runtime_config = {"providers": {"fixture": {
        "name": "fixture", "base_url": "https://fixture.example/v1",
        "api_key": "fixture-secret", "extra_body": {"fixture_keep": True},
        "max_output_tokens": 2048,
        "models": [{"id": "gpt-5.4", "vision": False}],
    }}}
    model, runtime = gateway_run._resolve_fallback_entry_agent_kwargs(
        {"provider": "fixture", "model": "gpt-5.4"}, runtime_config
    )
    assert runtime["request_overrides"]["extra_body"]["fixture_keep"] is True
    assert runtime["max_tokens"] == 2048
    runtime["fallback_service_tier_override"] = "normal"
    runner = object.__new__(GatewayRunner)
    runner._service_tier = "priority"
    ...
    with patch("run_agent.get_tool_definitions", return_value=[]), patch("run_agent.check_toolset_requirements", return_value={}), patch("run_agent.OpenAI"):
        agent = AIAgent(
            model=model, **route["runtime"], request_overrides=route["request_overrides"],
            quiet_mode=True, skip_context_files=True, skip_memory=True,
        )
    try:
        assert agent._active_fallback_service_tier_override == "normal"
        original = copy.deepcopy(agent.request_overrides)
        monkeypatch.setattr(agent, "_get_transport", lambda: CaptureTransport())
        wire = build_api_kwargs(agent, [{"role": "user", "content": "hi"}])
        assert "service_tier" not in wire["request_overrides"]
        assert wire["request_overrides"]["extra_body"]["fixture_keep"] is True
...
@pytest.mark.asyncio
async def test_native_manual_reset_failed_primary_preserves_pending_state(tmp_path, monkeypatch):
    from tests.gateway.test_compression_exhaustion_reset_policy import _make_runner
    ...
    for method in ("_evict_cached_agent", "_clear_conversation_scope",
                   "_begin_session_run_generation", "_is_session_run_current"):
        delattr(runner, method)
    source = _source()
    entry = store.get_or_create_session(source)
    key = entry.session_key
    assert store.set_session_metadata(key, _MANUAL_KEY, 0)
    state = runner._session_state(key)
    state.conversation.model_override = {"model": "temporary", "provider": "custom"}
    state.conversation.one_turn_restore = {"model_override": {"model": "standing"}}
    before = copy.copy(state.conversation)
    generation = state.persistent.run_generation
    resource = tmp_path / "old-resource"
    resource.write_text("owned by the old conversation")
    agent = SimpleNamespace(close=lambda: resource.unlink(), release_clients=MagicMock())
    runner._agent_cache[key] = (agent, "signature", 0, entry.session_id)
    primary = MagicMock(side_effect=OSError("primary failed"))
    monkeypatch.setattr(store._db, "replace_gateway_routing_entries", primary)
    monkeypatch.setattr("tools.async_delegation.interrupt_for_session", MagicMock())
    monkeypatch.setattr("tools.env_passthrough.clear_env_passthrough", MagicMock())
    monkeypatch.setattr("tools.credential_files.clear_credential_files", MagicMock())

    try:
        await runner._handle_message(_event("/new", source=source))
    except RuntimeError as exc:
        assert "persist" in str(exc)
    primary.assert_called_once()
    assert store.lookup_by_session_key(key).session_id == entry.session_id
    assert store.get_session_metadata(key, _MANUAL_KEY) == 0
    assert state.persistent.run_generation > generation
    assert {
        "conversation": state.conversation == before,
        "resource": resource.exists(),
        "cache": runner._agent_cache.get(key, (None,))[0] is agent,
    } == {"conversation": True, "resource": True, "cache": True}
    agent.release_clients.assert_not_called()


@pytest.mark.parametrize("stage,fail_primary", [("primary", False), ("primary", True), ("cleanup", False)])
@pytest.mark.asyncio
async def test_native_manual_reset_cancellation_settles_commit_and_cleanup(
    tmp_path, monkeypatch, stage, fail_primary
):
    from tests.gateway.test_compression_exhaustion_reset_policy import _make_runner

    runner, _ = _make_runner("none")
    with monkeypatch.context() as db_patch:
        store = _sqlite_store(tmp_path, db_patch)
    runner.session_store = store
    ...
    def barrier():
        entered.set()
        assert release.wait(3), "test did not release reset worker"
    ...
    agent = SimpleNamespace(close=close, release_clients=MagicMock())
    runner._agent_cache[key] = (agent, "signature", 0, entry.session_id)
    native_primary = store._db.replace_gateway_routing_entries
    ...
    def persist(*args, **kwargs):
        ...
        return native_primary(*args, **kwargs)

    primary = MagicMock(side_effect=persist)
    monkeypatch.setattr(store._db, "replace_gateway_routing_entries", primary)
    interrupt = MagicMock()
    monkeypatch.setattr("tools.async_delegation.interrupt_for_session", interrupt)
    monkeypatch.setattr("tools.env_passthrough.clear_env_passthrough", MagicMock())
    monkeypatch.setattr("tools.credential_files.clear_credential_files", MagicMock())
    ...
    lifecycle = MagicMock()
    monkeypatch.setattr("hermes_cli.lifecycle.invoke_hook", lifecycle)

    task = asyncio.create_task(runner._handle_message(_event("/new retained title", source=source)))
    try:
        assert await asyncio.to_thread(entered.wait, 2)
        task.cancel()
        await asyncio.sleep(0)
        task.cancel()
        await asyncio.sleep(0)
        unsettled = not task.done()
    finally:
        release.set()
        with pytest.raises(asyncio.CancelledError):
            await asyncio.wait_for(task, 3)
    assert unsettled, "cancellation escaped before the reset worker settled"
    primary.assert_called_once()
    current = store.lookup_by_session_key(key)
    with monkeypatch.context() as db_patch:
        reopened = _sqlite_store(tmp_path, db_patch)
    if fail_primary:
        assert current.session_id == entry.session_id
        assert reopened.get_session_metadata(key, _MANUAL_KEY) == 0
        ...
        runner._record_telegram_topic_binding.assert_not_called()
    else:
        assert current.session_id != entry.session_id
        assert reopened.lookup_by_session_key(key).session_id == current.session_id
        assert reopened.get_session_metadata(key, _MANUAL_KEY) is None
        assert state.conversation.one_turn_restore is None
        assert not state.conversation.model_override
        assert not resource.exists()
        assert key not in runner._agent_cache
        agent.release_clients.assert_called_once()
        interrupt.assert_called_once()
        assert [call.args[0] for call in runner.hooks.emit.await_args_list] == ["session:end", "session:reset"]
        runner._finalize_session_off_loop.assert_awaited_once()
        runner._session_db.set_session_title.assert_awaited_once_with(current.session_id, "retained title")
        runner._record_telegram_topic_binding.assert_called_once_with(source, current)
        assert any(call.args[0] == "on_session_reset" for call in lifecycle.call_args_list)
...
def test_native_limit_only_edit_rebuilds_cached_agent_and_request(
    tmp_path, monkeypatch, selected, before, after
):
    ...
    stores, agents, constructor_limits, requests = [], [], [], []
    # Stop only at the provider call; constructor, cache and request policy are native.
    class ProviderRequestCaptured(Exception):
        pass
    ...
    class RecordingAgent(AIAgent):
        ...
        def run_conversation(self, message, **kwargs):
            requests.append(self._build_api_kwargs([{"role": "user", "content": message}]))
            raise ProviderRequestCaptured

    config = {"providers": {"fixture": {
        "name": "fixture", "base_url": "https://fixture.example/v1",
        "api_key": "cache-fixture-key", "models": [{"id": "gpt-5.4", "vision": False}],
    }}, "fallback_providers": [{"provider": "fixture", "model": "gpt-5.4"}]}

    def write_limit(limit):
        config["providers"]["fixture"].pop("max_output_tokens", None)
        if limit is not None:
            config["providers"]["fixture"]["max_output_tokens"] = limit
        (home / "config.yaml").write_text(yaml.safe_dump(config))

    def check_budget(agent, limit):
        assert agent.max_tokens == limit
        assert agent.context_compressor.max_tokens == limit
        wire = requests[-1]
        limits = {k: v for k, v in wire.items() if k in {"max_tokens", "max_completion_tokens"}}
        assert list(limits.values()) == [65536 if limit is None else limit]
    ...
    try:
        ...
        runner.adapters = {}
        if not selected:
            # Same provider projection without manual selection is the ordinary-cache control.
            runner._resolve_session_agent_runtime = lambda **kw: gateway_run._resolve_fallback_entry_agent_kwargs(
                config["fallback_providers"][0], runtime_config=config)
        ctx = TurnContext(
            source=_source(), session_key=entry.session_key, session_id=entry.session_id,
            message="hi", history=[], AIAgent=RecordingAgent, enabled_toolsets=[],
            disabled_toolsets=[], user_config={"gateway": {"platforms": {
                "discord": {"skip_context_files": True}}}},
            resolve_display_setting=lambda *args: False,
            _hooks_ref=SimpleNamespace(loaded_hooks=False), _run_still_current=lambda: True,
        )
        turn = TurnRunner(runner, ctx)
        def run():
            with pytest.raises(ProviderRequestCaptured):
                turn.run_sync()
            return runner._agent_cache[entry.session_key][0]

        with patch("run_agent.OpenAI"), patch("run_agent.get_tool_definitions", return_value=[]), patch("run_agent.check_toolset_requirements", return_value={}):
            from hermes_cli.plugins import discover_plugins
            discover_plugins()
            first = run()
            assert constructor_limits == [before]
