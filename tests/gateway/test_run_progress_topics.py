...
@pytest.mark.asyncio
async def test_failed_heartbeat_task_preserves_native_turn_cleanup(monkeypatch, tmp_path):
    create_task = asyncio.create_task

    def create_with_failed_heartbeat(coro, *args, **kwargs):
        if getattr(getattr(coro, "cr_code", None), "co_name", None) == "_notify_long_running":
            coro.close()

            async def failed_heartbeat():
                raise RuntimeError("synthetic heartbeat transport failure")

            coro = failed_heartbeat()
        return create_task(coro, *args, **kwargs)

    monkeypatch.setattr(asyncio, "create_task", create_with_failed_heartbeat)
    adapter, result = await _run_with_agent(
        monkeypatch, tmp_path, FullTerminalArgsAgent,
        session_id="sess-native-heartbeat-cleanup",
        config_data={"display": {"tool_progress": "full"}},
    )
    assert result["final_response"] == "done"
    _, rendered = _first_progress_json(adapter)
...
def _residual_enqueue_args(args):
    ctx = TurnContext(
...
def test_residual_full_cli_credentials_preserve_shell_structure(monkeypatch, credential, masked):
    import agent.redact as redact
    ...
    safe = _residual_enqueue_args(args)
...
def test_residual_full_curl_headers_follow_shell_word_boundaries(monkeypatch, header, quote):
    import agent.redact as redact
    ...
    for option in ("--header=", "-H"):
        for value in ("opaque header 941", "opaque" + other_quote + "header"):
            ...
            original = deepcopy(args)
            assert _residual_enqueue_args(args) == {"command": prefix + masked + suffix}
            assert args == original
    ...
    for option in (quote + "--header" + quote + " ", quote + "-H" + quote + " ",
        ...
        assert _residual_enqueue_args({"command": prefix + argument + suffix}) == {
    ...
    assert _residual_enqueue_args({"command": "curl " + quote + "--header" + quote + "; echo public"}) == {
...
def test_residual_named_cli_forms_are_safely_redacted(label):
    vectors = {
    ...
    assert _residual_enqueue_args(args)["command"] == expected
