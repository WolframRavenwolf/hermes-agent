...
def native_web(monkeypatch):
    ...
    def configure(*, domains=None, **web):
...
@pytest.fixture
def native_parallel_web(native_web, monkeypatch):
    """Exercise native Parallel pairing; only emulate its SDK transport response."""
    from tools import web_tools
    configure, primary, rescue = native_web
    configure(extract_backend="parallel")
    monkeypatch.setenv("PARALLEL_API_KEY", "sdk-test")

    async def extract(*, urls, full_content):
        response = primary("parallel-sdk", json={"urls": urls})
        if response.status_code >= 400:
            raise ValueError(response.text)
        raw = response.json()
        documents = [SimpleNamespace(
            url=row.get("url"), title=row.get("title", ""),
            full_content=row.get("raw_content", ""), excerpts=[],
        ) for row in raw.get("results", [])]
        errors = [SimpleNamespace(url=url, content="extraction failed", error_type="failed")
                  for url in raw.get("failed_urls", [])]
        return SimpleNamespace(results=documents, errors=errors)

    monkeypatch.setattr(web_tools, "_async_parallel_client", SimpleNamespace(beta=SimpleNamespace(extract=extract)))
    return configure, primary, rescue
...
@pytest.mark.parametrize("kind", ["http_error", "exception", "all_error"])
...
def test_full_failures_keep_blocked_and_allowed_slots(native_web, kind, rescue_enabled):
    configure, primary, rescue = native_web
    configure(keyless_rescue=rescue_enabled)
    if kind == "http_error":
...
@pytest.mark.parametrize("route", ["normal", "http_error", "exception", "all_error"])
@pytest.mark.parametrize("blocked_final", [False, True])
def test_final_policy_precedes_storage_and_rescue_stays_uncached(native_parallel_web, route, blocked_final):
    configure, primary, rescue = native_parallel_web
...
def test_cache_rechecks_requested_and_final_current_policy(native_parallel_web, changed_host):
    configure, primary, rescue = native_parallel_web
    configure(domains=[])
...
@pytest.mark.parametrize("backend", ["firecrawl", "keenable", "tavily"])
...
def test_native_request_fallback_is_not_cache_provenance(native_web, keyed_firecrawl, monkeypatch, backend, reported):
    ...
    if backend == "firecrawl":
        ...
        primary.return_value = _response([{
            "title": "Page", "raw_content": "native page", **({"url": ALLOWED} if reported else {}),
        }])
        transport = primary
    else:
...
@pytest.mark.parametrize("reverse", [False, True])
@pytest.mark.parametrize("suffix", ["/", "?view=other"])
def test_distinct_trailing_slash_pages_keep_their_own_cached_bodies(native_parallel_web, reverse, suffix):
    _, primary, rescue = native_parallel_web
    bodies = {ALLOWED: "plain page", ALLOWED + suffix: "distinct page"}
    primary.return_value = _response([
...
def test_loose_rewrite_success_requires_one_exact_request_identity(native_parallel_web, shape):
    _, primary, rescue = native_parallel_web
...
def test_authenticated_requests_keep_order_duplicates_and_cache(native_parallel_web, rewritten):
    _, primary, rescue = native_parallel_web
    ...
    for url, final, body in zip(urls, finals, bodies):
        hit = cache.extract_cache_get(url, provider="parallel")
        assert hit is not None
        assert hit["final_url"] == final and hit["content"] == body
    rescue.assert_not_called()


def test_credential_stripped_batch_rows_never_own_authenticated_requests(native_parallel_web):
    _, primary, rescue = native_parallel_web
...
def test_pairing_keys_preserve_raw_case_sensitive_userinfo(template):
    from tools.web_tools import _url_key
