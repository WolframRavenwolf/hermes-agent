...
class TestTavilyExtractContracts:
    """Exercise transport ordering and policy through the real wrapper/cache."""
    ...
    def _isolated_tavily(self, monkeypatch):
        ...
        config = {
            ...
            "security": {"website_blocklist": {
                "enabled": True, "domains": ["blocked.example"], "shared_files": [],
            }},
        }
    ...
    def test_mixed_batch_does_not_cache_success_under_failed_url(self, failure_field):
        from tools.web_tools import web_extract_tool
        ...
        with patch("plugins.web.tavily.provider.httpx.post", return_value=_ok_response(raw)) as post:
            first = json.loads(asyncio.run(web_extract_tool([a, b])))["results"]
            later_a = json.loads(asyncio.run(web_extract_tool([a])))["results"][0]
            later_b = json.loads(asyncio.run(web_extract_tool([b])))["results"][0]
        assert later_a.get("error") and later_a.get("content") != "ONLY_PAGE_B"
        ...
        assert first[0].get("error") and first[1]["content"] == "ONLY_PAGE_B"

    def test_sparse_shuffled_response_preserves_requested_slots(self):
        from plugins.web.tavily.provider import TavilyWebSearchProvider
        ...
        with patch("plugins.web.tavily.provider.httpx.post", return_value=_ok_response(raw)):
            rows = TavilyWebSearchProvider().extract([a, c, b, a])
        assert [row["url"] for row in rows] == [a, c, b, a]
        assert rows[1].get("error") and not rows[1]["content"]
        assert [rows[i]["content"] for i in (0, 2, 3)] == ["A", "B", "A"]

    @pytest.mark.parametrize("keyless", [False, True])
    def test_policy_filters_transport_and_preserves_wrapper_flags(self, monkeypatch, keyless):
        from tools.web_tools import web_extract_tool
        from tools.web_result_cache import extract_cache_get
        ...
        blocked, allowed = "https://blocked.example/private", "https://example.com/allowed"
        assert check_website_access(blocked) is not None
        raw = {"results": [{"url": allowed, "raw_content": "ALLOWED"}]}
        with patch("plugins.web.tavily.provider.httpx.post", return_value=_ok_response(raw)) as post:
            rows = json.loads(asyncio.run(web_extract_tool([blocked, allowed, blocked])))["results"]
            assert post.call_args.kwargs["json"]["urls"] == [allowed]
        assert [r["url"] for r in rows] == [blocked, allowed, blocked]
        assert rows[0]["blocked_by_policy"]["rule"] == "blocked.example"
        assert rows[2]["blocked_by_policy"] == rows[0]["blocked_by_policy"]
        assert rows[1]["content"] == "ALLOWED"
        assert extract_cache_get(blocked, provider="tavily") is None
        assert extract_cache_get(allowed, provider="tavily")["content"] == "ALLOWED"

    @pytest.mark.parametrize("keyless", [False, True])
    def test_all_blocked_batch_never_calls_transport(self, monkeypatch, keyless):
        from tools.web_tools import web_extract_tool

        if keyless:
            monkeypatch.delenv("TAVILY_API_KEY")
        with patch("plugins.web.tavily.provider.httpx.post") as post:
            rows = json.loads(asyncio.run(web_extract_tool(["https://blocked.example/private"])))["results"]
        post.assert_not_called()
        assert rows[0]["blocked_by_policy"]["host"] == "blocked.example"

    @pytest.mark.parametrize("failure", [ValueError("provider unavailable"), RuntimeError("transport failure")])
    def test_transport_failure_preserves_policy_block(self, failure):
        from plugins.web.tavily.provider import TavilyWebSearchProvider

        blocked, allowed = "https://blocked.example/private", "https://example.com/allowed"
        with patch("plugins.web.tavily.provider.httpx.post", side_effect=failure) as post:
            rows = TavilyWebSearchProvider().extract([blocked, allowed])
        assert post.call_args.kwargs["json"]["urls"] == [allowed]
        assert rows[0]["blocked_by_policy"]["rule"] == "blocked.example"
        assert str(failure) in rows[1]["error"]
