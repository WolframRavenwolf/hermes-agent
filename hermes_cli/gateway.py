...
def generate_launchd_plist(app_wrapper: bool | None = None) -> str:
    ...
    if app_wrapper:
        data["AssociatedBundleIdentifiers"] = get_launchd_bundle_identifier()
    # Keep upstream's configurable FD floor (including its opt-out).
    try:
        ...
        nofile_target = None
    if nofile_target:
        data["SoftResourceLimits"] = {"NumberOfFiles": nofile_target}
...
def launchd_plist_is_current(app_wrapper: bool | None = None) -> bool:
    ...
    expected = generate_launchd_plist() if app_wrapper is None else generate_launchd_plist(app_wrapper=app_wrapper)
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        macos_app_wrapper = getattr(args, "macos_app_wrapper", None)
