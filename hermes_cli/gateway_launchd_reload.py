...
def _clear_launchd_reload_pending(plist_path: Path, attempt: str) -> str | None:
...
def _launchd_plist_update(plist_path: Path, definition: str):
    ...
    Only this pending attempt may compensate its published image. Restored bytes
    do not establish what launchd currently has loaded. After observed activation
    and matching-token unlink, retirement directory-sync failure is cleanup only.
    """
