...
class AIAgent:
    ...
    def _prepare_anthropic_messages_for_api(self, api_messages: list) -> list:
        ...
        # When native multimodal is enabled, pass image blocks through to the
        # Anthropic adapter which converts OpenAI image_url format to Anthropic's
        # native image source format.  Only fall back to text descriptions when
        # native multimodal is off (legacy behavior).
        try:
            from gateway.run import _load_gateway_config
            _cfg = _load_gateway_config()
            if _cfg.get("native_multimodal", False):
                return api_messages
        except Exception:
            pass
