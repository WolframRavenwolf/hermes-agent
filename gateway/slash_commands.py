...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        allow_full = "--full" in parts
        allow_cross_room = "--cross-room" in parts
        known_flags = {"--all", "--full", "--cross-room"}
        unknown_flags = [p for p in parts if p.startswith("--") and p not in known_flags]
        name_parts = [p for p in parts if p not in known_flags]
        name = " ".join(name_parts).strip()
        if unknown_flags and not name:
            # Preserve old fail-closed behavior for unknown flags: treat the
            # literal text as the lookup target so the user gets the normal
            # not-found response instead of accidentally widening scope.
        ...
        def _list_sessions_for_scope(*, cross_platform: bool, show_unnamed: bool, limit: int) -> list[dict]:
            ...
            if source.platform == Platform.MATRIX and not cross_platform:
                ...
                sessions = scoped
            if not show_unnamed:
                sessions = [s for s in sessions if s.get("title")]
            return sessions
        ...
        if not name:
            try:
                if not (allow_all or allow_full):
                ...
                sessions = _list_sessions_for_scope(
                    ...
                    show_unnamed=allow_full,
                    limit=50,
                )
                if not sessions:
                    scope = "across all platforms" if allow_all else "on this platform"
                    kind = "sessions" if allow_full else "named sessions"
                    return (
                ...
                header_kind = "All Sessions" if allow_full else "Named Sessions"
                lines = [f"📋 **{header_kind} — {scope_label}**\n"]
                for idx, s in enumerate(sessions[:20], start=1):
                    title = s.get("title")
                    preview = (s.get("preview") or "")[:40]
                    preview_part = t("gateway.resume.list_preview_suffix", preview=preview) if preview else ""
                    source_tag = f" [{s.get('source', '?')}]" if allow_all else ""
                    if title:
                        label = f"**{title}**"
                    else:
